...
# load data ---------------------------------------------------------------
df_date_range <- read_csv("covid-19_priority_measures_japan.csv")
...
for (i in seq_along(unique_id)) {
  ...
  for (j in 1:dim(df_date_range_i)[1]){
    start_date <- df_date_range_i$priority_measures_start[j]
    end_date <- df_date_range_i$priority_measures_end[j]
